...
Spring Bean은 기본적으로 **싱글톤(Singleton)** 스코프다.
즉, 컨테이너 내에서 해당 Bean의 인스턴스가 **딱 하나**만 생성되고, 모든 요청에서 그 하나를 공유한다.

```java
@Service
public class KisAuthService {
    private String cachedToken;  // 이 필드는 서버 전체에서 하나
}
```

```
첫 번째 요청 → KisAuthService 인스턴스 생성 (이후 재사용)
두 번째 요청 → 같은 인스턴스 반환 (새로 생성 X)
열 번째 요청 → 같은 인스턴스 반환
```

→ `cachedToken`이 싱글톤 Bean의 필드이기 때문에 한 번 저장되면 서버가 살아있는 동안 유지됨
→ 이것이 메모리 캐싱이 가능한 이유

### 싱글톤의 주의점

상태(state)를 가지는 필드는 조심해야 한다.
여러 요청이 동시에 같은 인스턴스를 공유하기 때문에 **동시성 문제**가 생길 수 있다.

```java
// 위험한 예 — 여러 사용자가 동시에 요청하면 cachedToken이 덮어씌워질 수 있음
private String cachedToken;
```

현재 pong-to-rich는 **단일 계정(나 혼자)** 사용이라 문제없음.
다중 사용자 지원 시 → Redis로 `userId → token` 형태로 확장 필요

### 다른 스코프

| 스코프 | 설명 | 사용 시점 |
|--------|------|----------|
| `singleton` | 기본값. 컨테이너당 인스턴스 1개 | 대부분의 Bean |
| `prototype` | 요청마다 새 인스턴스 생성 | 상태를 가지는 객체 |
| `request` | HTTP 요청당 1개 (웹 환경) | 요청별 데이터 |
| `session` | HTTP 세션당 1개 (웹 환경) | 세션별 데이터 |
...
| 싱글톤 | Bean의 기본 스코프. 인스턴스 하나를 모든 요청이 공유 |
